...
public class PassengersData : ScriptableObject
{
...
public class HabitFolderObserver : AssetPostprocessor
{
    ...
    {
        string[] guids = AssetDatabase.FindAssets("t:" + nameof(PassengersData));

        for (int i = 0; i < guids.Length; i++)
        {
            string guid = guids[i];
            string assetPath = AssetDatabase.GUIDToAssetPath(guid);
            PassengersData passengerData = AssetDatabase.LoadAssetAtPath<PassengersData>(assetPath);
